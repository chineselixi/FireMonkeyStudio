...
protected:
    bool eventFilter(QObject *obj, QEvent *ev) {
        if (ev->type() == QEvent::KeyPress) {
            ...
            doc->replaceSelectedText((char)kev->key() + text + (char)other);
            return true;
        }
        return false;
    }
...
void Sqriptor::loadFile(const QString &fileName)
{
    ...
    if (!(codec.isEmpty() || codec.contains("utf-8", Qt::CaseInsensitive) ||
        ...
        if (codec.contains("utf-16", Qt::CaseInsensitive))
            ...
            QString codecName = ask4Codec(codec, fileName);

            std::optional<QStringConverter::Encoding> encodingOpt = QStringConverter::encodingForName(codecName.toLatin1().data());
