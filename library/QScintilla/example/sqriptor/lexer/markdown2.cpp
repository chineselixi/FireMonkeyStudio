...
#define NEXT_IS_WS i.hasNext() && !nonWS.match(i.peekNext().capturedView(0)).hasMatch()
        ...
        if (tag) {
            ...
            if (i.hasNext() && i.peekNext().capturedView(0) == QLatin1String("=")) {
        ...
        if (newline && !quote) {
            if (*token.data() == "######" && NEXT_IS_WS) {
                finishStyle(); newline = false; header = 6;
            } else if (*token.data() == "#####" && NEXT_IS_WS) {
            ...
            } else if (*token.data() == "---" && NEXT_IS_WS) { // TODO: not entirely accurate,  ---foobar is not a ruler, quotes?
                finishStyle(); newline = false; setStyling(3, Style::Ruler);
                continue;
            } else if ((*token.data() == "*" || *token.data() == "-" || *token.data() == "+") && NEXT_IS_WS) {
                ...
                if (isInt && i.hasNext() && i.peekNext().capturedView(0) == QLatin1String(".")) { // ordered list item
        ...
        if (*token.data() == "[") {
            finishStyle();
            bracket = true;
        } else if (*token.data() == "(" && !length && prev.capturedView() == QLatin1String("]")) {
