...
public:
    ...
    virtual void onSubWidgetEnter(QWidget* packWidget,QWidget* subWidget);//子控件进入(参数1为容器控件，是本插件插件的。参数2为进入的子控件)
    virtual QList<QAction*> getMenuAction();//系统获取插件的右键菜单
    virtual void onMenuActionClick(QAction* action,widgetMsg* widget);//菜单选项被按下
