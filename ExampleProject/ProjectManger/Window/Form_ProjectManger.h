...
private:
    ...
    ProjectMsg readProjectMsg(QString fmsFilePath); //读取FMS信息
...
public:
    ...
    ProjectMsg getNowProjectMsg(); //获取当前工程
    ProjectMsg getProjectMsg(QString proPath); //获取当前工程信息
    QString getCreatePath(QString srcPath); //根据一个文件或者文件夹路径，获取已经可用的文件夹路径
    bool saveProjectMsg(QString proPath, ProjectMsg proMsg); //保存指定工程信息
