...
//读取FMS信息
Form_ProjectManger::ProjectMsg Form_ProjectManger::readProjectMsg(QString fmsFilePath)
{
    QJsonDocument t_jsonDoc = QJsonDocument::fromJson(System_File::readFile(fmsFilePath));
    if(!t_jsonDoc.isObject()) return ProjectMsg();
    ProjectMsg t_retMsg;
...
//获取当前工程
Form_ProjectManger::ProjectMsg Form_ProjectManger::getNowProjectMsg()
{
    if(this->nowProject){
...
//获取当前工程信息
Form_ProjectManger::ProjectMsg Form_ProjectManger::getProjectMsg(QString proPath)
{
    QString t_savePath = "";
...
//保存当前工程信息
bool Form_ProjectManger::saveProjectMsg(QString proPath, ProjectMsg proMsg)
{
    QString t_savePath = "";
    t_savePath = this->getCreatePath(proPath); //获取一个有效的创建路径
